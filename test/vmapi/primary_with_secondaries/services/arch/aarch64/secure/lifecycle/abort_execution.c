...
/*
 * Retrieve a read-only lent page, validate read access, then attempt a
 * write after making stage-1 writable so the abort is a direct stage-2
 * permission fault.
 */
TEST_SERVICE(ffa_memory_retrieve_permission_fault)
{
	void *recv_buf = SERVICE_RECV_BUFFER();
	void *send_buf = SERVICE_SEND_BUFFER();
	struct ffa_memory_region *memory_region =
	...
	exception_setup(NULL,
			exception_handler_yield_data_abort_permission_fault);
	retrieve_memory_from_message(recv_buf, send_buf, &handle, memory_region,
				     sizeof(sp_retrieve_buffer));
	composite = ffa_memory_region_get_composite(memory_region, 0);
	...
	/*
	 * Read access should succeed after retrieving the lent page as RO.
	 */
	for (i = 0; i < composite->constituent_count; ++i) {
		// NOLINTNEXTLINE(performance-no-int-to-ptr)
		volatile uint8_t *ptr = (uint8_t *)constituents[i].address;
		uint32_t count = constituents[i].page_count;
		size_t j;
	...
	/*
	 * Yield once so the primary can reschedule us for the faulting write.
	 */
	EXPECT_EQ(ffa_yield().func, FFA_SUCCESS_32);

	/*
	 * Force stage-1 to permit the write; the retrieved memory is still RO
	 * in stage-2, so the write must fault as a direct stage-2 permission
	 * fault.
	 */
	for (i = 0; i < composite->constituent_count; ++i) {
		hftest_mm_identity_map(
			// NOLINTNEXTLINE(performance-no-int-to-ptr)
			(const void *)constituents[i].address,
			PAGE_SIZE * constituents[i].page_count,
			MM_MODE_NS | MM_MODE_R | MM_MODE_W);
	}

	// NOLINTNEXTLINE(performance-no-int-to-ptr)
	((volatile uint8_t *)constituents[0].address)[0]++;
