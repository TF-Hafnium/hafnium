...
#define HFTEST_SET_UP(suite_name)                                         \
	...
	[[gnu::used]] [[gnu::section(HFTEST_SET_UP_SECTION(suite_name))]] \
	const struct hftest_test HFTEST_SET_UP_STRUCT(suite_name) = {     \
		.suite = #suite_name,                                     \
...
#define HFTEST_TEAR_DOWN(suite_name)                                         \
	...
	[[gnu::used]] [[gnu::section(HFTEST_TEAR_DOWN_SECTION(suite_name))]] \
	const struct hftest_test HFTEST_TEAR_DOWN_STRUCT(suite_name) = {     \
		.suite = #suite_name,                                        \
...
#define HFTEST_TEST(suite_name, test_name, long_running, precon_fn)            \
	...
	[[gnu::used]] [[gnu::section(                                          \
		HFTEST_TEST_SECTION(suite_name, test_name))]]                  \
	const struct hftest_test HFTEST_TEST_STRUCT(suite_name, test_name) = { \
		.suite = #suite_name,                                          \
...
#define HFTEST_SERVICE_SET_UP(service_name)                       \
	...
	[[gnu::used]] [[gnu::section(                             \
		HFTEST_SERVICE_SET_UP_SECTION(service_name))]]    \
	const struct hftest_test HFTEST_SERVICE_SET_UP_STRUCT(    \
		service_name) = {                                 \
		.name = #service_name,                            \
...
#define HFTEST_TEST_SERVICE(service_name)                                    \
	...
	[[gnu::used]] [[gnu::section(HFTEST_SERVICE_SECTION(service_name))]] \
	const struct hftest_test HFTEST_SERVICE_STRUCT(service_name) = {     \
...
#define HFTEST_SERVICE_SELECT(vm_id, service, send_buffer, vcpu_id)           \
	do {                                                                  \
		 ...
		 * handled.                                                   \
		 */                                                           \
		ffa_rxtx_header_init(&message->header, hf_vm_get_id(), vm_id, \
				     msg_length);                             \
