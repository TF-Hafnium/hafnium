 ...
 ******************************************************************************/
static unsigned int gicv3_get_espi_limit(uintptr_t gicd_base)
{
...
#if GIC_EXT_INTID
	if ((read_gicd_typer_reg(plat_gicv3_driver.dist_base) & TYPER_ESPI) !=
	    TYPER_ESPI) {
		dlog_warning(
			"GIC extended SPI support built in but hardware "
			"doesn't support it.\n");
	} else {
